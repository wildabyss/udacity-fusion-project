...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            for sample_i in range(len(output_post)):
                # Loop over vehicles only
                detection = output_post[sample_i][1]
                for obj in detection:
    ...
    if len(detections) > 0:
        ...
        for obj in detections:
            ...
            x = obj[2] * (configs.lim_x[1] - configs.lim_x[0]) / configs.bev_height
            # Corresponding to the definition of bev_discret in bev_from_pcl
            y = (obj[1] - (configs.bev_width + 1)/2) * (configs.lim_x[1] - configs.lim_x[0]) / configs.bev_height
            obj[1] = x
            obj[2] = y
            obj[3] = obj[3] + configs.lim_z[0]
            obj[5] = obj[5] * (configs.lim_y[1] - configs.lim_y[0]) / configs.bev_width
            obj[6] = obj[6] * (configs.lim_x[1] - configs.lim_x[0]) / configs.bev_height
            obj[7] = -obj[7]
